...
module.exports = function makeWebpackConfig() {
  ...
  config.output = isTest ? {} : {
    path: root('dist'),
    publicPath: isProd ? 'http://localhost:8080/' : 'http://localhost:2016/',
  ...
  config.module = {
    rules: [
      ...
      {
        test: /\.ts$/,
        loaders: ['awesome-typescript-loader?' + atlOptions, 'angular2-template-loader', '@angularclass/hmr-loader','lodash'],
  ...
  if (!isTest && !isTestWatch) {
    config.plugins.push(
      ...
      new ExtractTextPlugin({filename: 'style/[name].css'})//({filename: 'css/[name].css', disable: !isProd})
    );
  }
